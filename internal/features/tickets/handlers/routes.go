...
func RegisterRoutes(mux *http.ServeMux, deps Deps) {
	...
	mux.Handle(
		"GET /tickets/new",
		middleware.RequireAuth(memberOnly(http.HandlerFunc(h.ticketsNewGET))),
	)
	mux.Handle(
		"POST /tickets",
		middleware.RequireAuth(memberOnly(http.HandlerFunc(h.ticketsCreatePOST))),
	)
	mux.Handle(
		"GET /tickets/{id}",
		middleware.RequireAuth(memberOnly(http.HandlerFunc(h.ticketDetailGET))),
	)
	mux.Handle(
		"GET /tickets/inbox",
		middleware.RequireAuth(syndicOnly(http.HandlerFunc(h.inboxGET))),
	)
	mux.Handle(
		"POST /tickets/{id}/reply",
		middleware.RequireAuth(syndicOnly(http.HandlerFunc(h.ticketReplyPOST))),
	)
	mux.Handle(
		"POST /tickets/{id}/close",
		middleware.RequireAuth(syndicOnly(http.HandlerFunc(h.ticketClosePOST))),
	)
}
