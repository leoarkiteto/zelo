...
func (s *FinanceService) ListAccounts(ctx context.Context, condominiumID string, f ports.AccountFilter) ([]domain.FinancialAccount, error) {
	return s.Accounts.List(ctx, condominiumID, f)
}

// Summary returns the dashboard projection for the month (FR-013).
func (s *FinanceService) Summary(ctx context.Context, condominiumID string, month time.Time) (ports.MonthSummary, error) {
...
func (s *FinanceService) Health(ctx context.Context, condominiumID string, month time.Time) (ports.MonthSummary, error) {
	return s.Summaries.SummaryForMonth(ctx, condominiumID, month)
}
