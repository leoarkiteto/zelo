...
func registrationServiceFixture(users *fakeUserStore, roles *fakeRoleStore, invitations *fakeInvitationStore) *RegistrationService {
	return &RegistrationService{
		...
		HashPassword:             testHasher{}.Hash,
		ValidatePassword:         testHasher{}.ValidatePassword,
		HashToken:                testHasher{}.HashToken,
		Now:                      time.Now,
	}
}
...
func TestRegistrationServiceRegisterValid(t *testing.T) {
	...
	svc := registrationServiceFixture(userStore, roleStore, invStore)
...
func TestRegistrationServiceRegisterInvalidInvitation(t *testing.T) {
	...
	svc := registrationServiceFixture(newFakeUserStore(), &fakeRoleStore{}, invStore)
	err := svc.Register(context.Background(), token, "owner@example.com", "valid-password-123")
	...
	svc.GetInvitationByTokenHash = used.GetInvitationByTokenHash
...
func TestRegistrationServiceRegisterDuplicateEmail(t *testing.T) {
	...
	svc := registrationServiceFixture(userStore, &fakeRoleStore{}, invStore)
	err := svc.Register(context.Background(), token, "owner@example.com", "valid-password-123")
