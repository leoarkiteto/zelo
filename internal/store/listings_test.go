...
func TestCategoryStoreCreateAndRename(t *testing.T) {
	...
	if _, err := store.CreateCategory(ctx, model.ServiceCategory{CondominiumID: condoID, Name: "Pipe Repair"}); !errors.Is(err, ErrDuplicate) {
...
func TestListingStoreLifecycle(t *testing.T) {
	...
	if _, err := store.GetListingByID(ctx, id); !errors.Is(err, ErrNotFound) {
